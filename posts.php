...
if (!isset($_GET['tag'])||(isset($_GET['tag'])&&$_GET['tag']==1)) {
    ...
    if (isset($_GET['pag'])) {
        ...
        if ($pag <= ceil(intval($cant["c"]) / 4)) {
            $in = ($pag * 4) - 4;
            $sql = "SELECT * FROM posts AS p INNER JOIN tag_post AS tp ON tp.post_id = p.id AND tp.tag_id = '".$_GET['tag']."' ORDER BY p.fecha_alta DESC LIMIT $in,4";
        ...
        $sql = "SELECT * FROM posts AS p INNER JOIN tag_post AS tp ON tp.post_id = p.id AND tp.tag_id = '".$_GET['tag']."' ORDER BY p.fecha_alta DESC LIMIT 0,4";
